...
if($_GET['send']=='true'){
	...
	$sql = "SELECT `blok_rumah`, `nomor_rumah`, `nama_suami`, `nama_istri`,`telepon_suami`,`telepon_istri`,id_warga 
						FROM t_warga 
						WHERE nama_suami<>'belum' AND tgl_keluar<1 $where ORDER BY `blok_rumah`, CAST( nomor_rumah AS INTEGER) ASC ";
	//echo $sql;
	$hasil = $db->query($sql);
	$n = 0;
	while($row = $hasil->fetchArray()){
		...
		if($_POST['test']=='yes' && $lanjut){
			...
			if($config->Gunakan_Blok=='true'){
				$nama .= " [".$row['blok_rumah']."-".$row['nomor_rumah']."]";
			}
			
			//$status = file_get_contents("http://127.0.0.1:9090/sendsms?phone=$nomorhp&text=".urlencode($sms));
			$dbSMS->query("insert into t_outbox(status,id_warga,nama_warga,nomor_warga,isi_sms),
			values('',".$row['id_warga'].",'$nama','$nomorhp','$sms')");
...
<div class="btn-group" role="group">
	<input name="" type="submit" class="btn btn-primary" value="Compose SMS">
</div>
<div class="btn-group" role="group">
	<a href="?apa=SMSOutbox" class="btn btn-info">Outbox</a>
</div>
</div>
</form>
<script>
